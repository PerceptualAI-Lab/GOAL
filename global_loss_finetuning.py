...
def main(args):
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # Use only GPU index 1
...
def train(
    ...
    for epoch in range(args.epochs):
        ...
        # Calculate and log average loss at the end of epoch
        ...
        save_path = os.path.join(args.output_dir, f"Long_{os.path.splitext(os.path.basename(args.model))[0]}_{os.path.splitext(os.path.basename(args.dataset))[0]}_{epoch+1}_{args.image_size}.pth")
        ...
        if fabric.global_rank == 0:
            ...
            # Move state_dict to CPU and save
...
def get_args_parser():
    ...
    parser.add_argument('--dataset', default='datasets/DCI_train_del_org.json', type=str)
    ...
    parser.add_argument('--output_dir', default='finetune_out_DCI/long_batch16_base16',
