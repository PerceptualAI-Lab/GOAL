...
def get_text_tokens_from_segment(text_tokens, org_text, seg_text, processor):
    """
    Args:
        text_tokens: (B, L, D) tensor of text tokens - original text의 전체 토큰
    ...
    seg_tokens = processor(text=seg_text,
                         ...
                         padding=False,
                         truncation=False)
    seg_token_length = len(seg_tokens.input_ids[0]) - 2  # CLS, EOS 토큰 제외

    if sent_idx != -1:
        # 문장 위치를 기반으로 토큰 인덱스 계산
    ...
    if start_idx >= max_length:
        # segment가 최대 길이를 넘어선 위치에 있는 경우,
        # segment 길이만큼의 공간을 확보하여 뒤에서부터 토큰 추출
        end_idx = max_length - 1
        start_idx = max(1, end_idx - seg_token_length)  # 1은 CLS 토큰 다음부터 시작
    else:
        # 정상적인 범위 내에 있는 경우
...
class DLoader(Dataset):
    ...
    def _load_image(self, name):
        ...
        return img, img.size  # 원본 이미지 크기도 반환
    ...
    def __getitem__(self, idx):
        ...
        org_image = self._load_image(item["original_filename"])[0]  # size 정보는 필요 없음
...
def train(fabric: L.Fabric, model: torch.nn.Module, optimizer: torch.optim.Optimizer, train_loader, processor) -> None:
    ...
    for epoch in range(args.epochs):
        ...
        for i, samples in enumerate(train_loader):
            ...
            sim_patch = patch_pooled @ seg_image_embeds.t()  # logit_scale 제거
            ...
            for b in range(batch_size):
                ...
                #print("\nOriginal Tokens:")
                #print(org_tokens_text)
                #print("\nSegment Tokens:")
                #print(seg_tokens_text)
                
                # get_text_tokens_from_segment 함수로 추출된 토큰의 위치 확인
                start_idx = len(processor(text=org_caption[b][:org_caption[b].find(seg_caption[b])], 
                                        return_tensors="pt", 
                                        padding=False, 
                                        truncation=False).input_ids[0])
                
                end_idx = start_idx + len(seg_tokens.input_ids[0]) - 2  # CLS, EOS 토큰 제외
                
                #print("\nExtracted Token Range:")
                #print(f"Start index: {start_idx}")
                #print(f"End index: {end_idx}")
                #print("\nExtracted Tokens:")
                #print(org_tokens_text[start_idx:end_idx])
            ...
            sim_text = text_pooled @ seg_text_embeds.t()  # logit_scale 제거
            ...
            if fabric.global_rank == 0:
                wandb.log({
                    ...
                    "patch_similarity": patch_diag.mean().item(),  # 평균 패치 유사도
                    "text_similarity": text_diag.mean().item(),    # 평균 텍스트 유사도
                })
...
def get_args_parser():
    ...
    parser.add_argument('--init_lr', type=float, default=5e-6, metavar='LR') # 원래 5e-6
